...
parameters {
    ...
    real<lower=0,upper=1.0/dt_norm> gamma_max[m-1]; 
    real<lower=0> rho_mu; 
    real<lower=0> rho_sigma; 
    real<lower=0,upper=1.0/dt_norm> rho_max[m-1]; 
    ...
    real<lower=0> cell_count[nt];       // record relative cell count for each time step 
    {
        ...
        for (it in 1:nt){ // time-stepping loop
            ...
            for (i in 1:m){ // size-class loop
                ...
                if (i < m){
                    gamma = dt_norm * gamma_max[i] * (1.0 - exp(-E[it]/E_star));
                    gamma_sat = dt_norm * gamma_max[i];
                } else {
                    gamma = 0.0; // value is not used
                    gamma_sat = 0.0;  // value is not used
                }
                if (i > 1){
                    rho = dt_norm * rho_max[i-1];
                } else {
                    rho = 0.0; //value is not used
                }
...
model {
    ...
    gamma_mu ~ normal(10.0, 10.0) T[0,1.0/dt_norm];
    gamma_sigma ~ exponential(0.1);
    gamma_max ~ normal(gamma_mu, gamma_sigma);
    rho_mu ~ normal(3.0, 10.0) T[0, 1.0/dt_norm];
    rho_sigma ~ exponential(0.1);
    rho_max ~ normal(rho_mu, rho_sigma);
